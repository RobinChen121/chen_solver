...
namespace
{
    ...
    {
        ActivityRange range;
        // 把每个变量可能的最小值和最大值代入约束条件，计算出约束条件的最小活动值和最大活动值
...
PresolveReport presolveLP(const ChenModel& model)
{
    ...
    for (const auto& constraint : constraints)
    {
        ...
        // 当用 push_back 直接传入大对象时，会发生拷贝构造，导致性能下降。使用 std::move 可以避免不必要的拷贝，提高性能。
        rows.push_back(std::move(row)); // 使用 move 避免不必要的拷贝
    }
    ...
    do
    {
        ...
        for (auto& row : rows)
        {
            ...
            {
                // 过大或过小的上下界没必要 shift，因为它们不会影响约束条件的可行性
            ...
            if (row.terms.empty())
            {
                // empty 是 vector 自带的判断是否为空容器的函数
                if ((!isNegInf(row.lb) && row.lb > EPS) || (
                    !isPosInf(row.ub) && row.ub < -EPS)) // 空条件仍然出现上下界的情况
                {
                    report.result = PresolveResult::PrimalInfeasible;
                    return report;
                }
            ...
            // min_is_neg_inf 只要约束条件有一个变量的最小值是负无穷，那么整个约束条件的最小活动值就是负无穷
            ...
            // 处理单变量约束条件 (singleton constraint)
            const auto [col, coef] = row.terms.front(); // front() 返回 vector 的第一个元素的引用，适用于只有一个元素的情况
