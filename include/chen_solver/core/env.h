...
public:
    ...
    // --- setters ---
    ChenEnv& setTimeLimit(const double seconds)
    {
        time_limit_ = seconds;
        return *this;
    }

    ChenEnv& setThreads(const int n)
    {
        threads_ = n;
        return *this;
    }

    ChenEnv& setOutputFlag(const bool flag)
    {
        output_flag_ = flag;
        return *this;
    }

    ChenEnv& writeLog(const std::string& path)
    {
        log_file_path_ = path;
        return *this;
    }

    ChenEnv& setMipGap(const double gap)
    {
        mip_gap_ = gap;
        return *this;
    }

    ChenEnv& setFeasibilityTol(const double tol)
    {
        feasibility_tol_ = tol;
        return *this;
    }

    ChenEnv& setIntFeasTol(const double tol)
    {
        int_feas_tol_ = tol;
        return *this;
    }
